...
stbi_uc* loadImage(const char* path_to_image, int* width, int* height, int* channels, int desired_channels) {
    return stbi_load(path_to_image, width, height, channels, desired_channels);
}
...
void writeImage(const char* path_to_image, stbi_uc* image, int width, int height, int channels) {
    ...
    stbi_write_bmp(path_to_image, width, height, channels, image);
}
...
__host__ __device__ void getPixel(stbi_uc* image, int width, int x, int y, Pixel* pixel) {
    const stbi_uc* p = image + (STBI_rgb * (y * width + x));
    pixel->r = p[0];
    pixel->g = p[1];
    pixel->b = p[2];
    // pixel->a = p[3];
...
__host__ __device__ void setPixel(stbi_uc* image, int width, int x, int y, Pixel* pixel) {
    stbi_uc* p = image + (STBI_rgb * (y * width + x));
    p[0] = pixel->r;
    p[1] = pixel->g;
    p[2] = pixel->b;
    // p[3] = pixel->a;
